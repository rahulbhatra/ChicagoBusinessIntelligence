...
const tableColumns =  ['id', 'lab_report_date', 'cases_total', 'death_total'];
const chartColumns = [
    { value: 'cases_total', name: 'Cases Total' },
    { value: 'death_total', name: 'Dealth Total' }
];
const chartArgumentField = "lab_report_date";
...
const CovidDaily = () => {
    ...
    const getData = async () => {
        await axios.get('http://localhost:4000/api/covid_daily/data', {})
        .then(res => {
            setToastOpen(true);
